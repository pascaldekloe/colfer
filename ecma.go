...
{{- end}}
{{range .}}
{{.DocText "//"}}var {{.NameNative}} = new function() {
...
{{range .Structs}}
	// Constructor.
{{.DocText "\t//"}}	// When init is provided all enumerable properties are merged into the new object a.k.a. shallow cloning.
	this.{{.NameTitle}} = function(init) {
{{- range .Fields}}
{{.DocText "\t\t//"}}		this.{{.NameNative}} = {{if .TypeList}}[]
